---
sidebar_position: 3
description: How to complete your manuscript review as a manager.
---
import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';
import TurnaroundMap from '@site/src/components/TurnaroundMap';

# Manuscript Management Review

## My Manuscript Management Reviews Page {/* #my-manuscript-management-reviews-page */}

You can check the status of manuscripts you have reviewed or have been selected to review on the **My Manuscript Management Reviews** page.

To access the **My Manuscript Management Reviews** page:

1. Expand the page selection menu by hovering your mouse over the left-sidebar menu.
2. Select **My Manuscript Management Reviews**.

:::note
In cases where the review involves revisions to the manuscript, you may notice
that the same manuscript appears for each step of that review. This is because
each step has its own tracked review outcome.
:::

On the **My Manuscript Management Reviews** page, you will see a list of all manuscripts awaiting your review. Click on a pending Manuscript Record Form (MRF) to begin reviewing it.

![My Manuscript Management Reviews](./images/my_management_reviews.png)

## Reviewing an MRF {/* #reviewing-an-mrf */}

![Manuscript Management Reviews](./images/management_review.png)

### Your role as the reviewing manager {/* #your-role-as-the-reviewing-manager */}

Reviews the manuscript for compliance with the [Intellectual Property Policy/Copyright Act](https://www.dfo-mpo.gc.ca/terms-avis/copyright-droits-eng.htm), the [Privacy Act](https://www.priv.gc.ca/en/privacy-topics/privacy-laws-in-canada/the-privacy-act/pa_brief/), the Financial Administration Act (with respect to approvals of publication costs) and the [Values and Ethics Code for DFO](https://intranet.ent.dfo-mpo.ca/hr-rh/en/node/1172).

Identifies when a manuscript is of potential public interest for the sole purpose of informing RDS and DFO regional Communications. Examples of topics of potential public interest could include subjects related to current events, or of stakeholder interest. These can include interesting stories to showcase DFO research or studies that are more controversial. The inclusion of potentially sensitive material will never prevent the publication of a manuscript in any way.

Authors may submit their publication to the third-party journal for peer review after the manager has deemed the publication to be in alignment with the relevant legislation, policies and directives, or if no feedback was received from the manuscript management review within 10 working days. Division Managers, EOS Directors in the National Capital Region (NCR), Regional Directors of Science and National Directors General may perform the manuscript management review.

For more information please refer to the [Fisheries and Oceans Canada National Policy for Science Publications](https://intranet.ent.dfo-mpo.ca/science/en/node/1471).

### 10-working-day timeline {/* #10-working-day-timeline */}

For convenience, the portal calculates the 10-working-day timeline where applicable. However, the portal does not automatically review MRFs past this time and managers are still expected to complete the review regardless of whether the paper was submitted.

<Tabs>

<TabItem value="third-party-journal-publications" label="Third-Party Journal Publications" default>

If a manager identifies issues related to the relevant legislation, policies, and directives, they must communicate with the author(s) to request revisions or refer the publication to the respective Regional Director of Science or National Director General, and the 10-working-day timeline is paused.

Example of paused timeline:

- A division manager sends the manuscript to an RDS or DG.
- A manager asks the author for revisions based on their review. In this case, the 10-working-day timeline restarts when the author replies.
</TabItem>
<TabItem value="dfo-publications" label="DFO Publications">

There is no mandatory 10-working-day timeline for DFO publications. As per the policy, managers are to reply in a timely manner.

A Region's RDS can opt the Region into a 10-working-day timeline for DFO Publications. DFO Publications will receive a 10-working-day review deadline with due and overdue reminders.  
To Opt-in please email the [OSP Support Team](mailto:DFO.OpenScience-ScienceOuverte.MPO@dfo-mpo.gc.ca).
</TabItem>
</Tabs>

### Optional 10 business day turnaround by region {/* #optional-10-business-day-turnaround-by-region */}
The map indicates whether the region has adopted the optional 10 business day turnaround for secondary publications.

<TurnaroundMap />


### Review the Manuscript Form {/* #review-the-manuscript-form */}

:::tip
While you are the current reviewer, you are able to edit the MRF form as required.
:::

The Manuscript and any Potential Public Interest can be reviewed in the Quick Review panel in the Manuscript Management Review page.

Review the Manuscript PDF.

**Steps**
1. Click the Download Icon at the right-side of the Manuscript PDF box.
2. Open the PDF from your computers Downloads folder.

Review the Manuscript Record Form.

**Steps**
1. Click **\<- GO TO MANUSCRIPT FORM**.
2. Review the MRF page.
3. Apply desired edits to the MRF (Optional)

If **Edits** are made:
- Record any edits made in the **Management Comments**!

Review Potential Public Interest.

**Steps**
1. Click **\<- GO TO POTENTIAL PUBLIC INTEREST SECTION**
2. Review the Potential Public Interest within the MRF page.
3. Apply desired edits to the Potential Publical Interest (Optional).

If **Edits** are made:
- Record any edits made in the **Management Comments**

**Result**

The Manuscript Management Review has been completed.

![Reivew with Interest](./images/review_with_interest.png)

#### Management Comments {/* #management-comments */}

If you intend to forward or reassign the manuscript management review to another manager, or if you have feedback you would like the author to address, you must record your comments in the **Manager Comments** textbox. These comments will be visible to anyone with current or future access to this manuscript record.

If you intend to approve and complete the manuscript management review, comments are not required.

![Management Comments](./images/management_comments.png)

## Manuscript Management Review Actions {/* #manuscript-management-review-actions */}

### Review Decision {/* #review-decision */}

:::important

- For DFO publications, only an RDS, DG or user that has been assigned the `director` role within the portal may complete the manuscript management review.
- A manager comment is required for all actions other than to `complete` the manuscript management review.

:::

![Manuscript Management Reviews](./images/decision.png)

The available management decisions are:

- **Complete**
  :::info
  For DFO Publications, only a user with the Director role can perform this action.
  :::
  - You've reviewed this manuscript for publication and are ending the manuscript management review process.

- **Revision required**
  - You flag manuscript management review comments that the author should address before providing your approval.

- **Refer to next manager**
  - You have reviewed this manuscript and are referring it to the next manager (usually an RDS or DG) for review.
  - This step stops the 10-working-day timeline for third-party and preprint manuscripts.
  :::tip
  To help speed up the review and increase transparency, always ensure to
  include clear notes and recommendations for the next manager. This is especially important if you think that a revision is required.
  :::

- **Reassign**
  - You are not the appropriate manager to review this manuscript and reassign the review to the correct manager without
  :::tip
  If you are an author who has mistakenly sent the manuscript for review to the
  wrong recipient, you can also contact the portal team for help.
  :::

### Planning Binder {/* #planning-binder */}

A MRF **must** have [Potential Public Interest](./manuscript_record_form.mdx#indicate-potential-public-interest) in order to be flagged for the Planning Binder within the OSP.

If the MRF should be considered an item on the EOS ADM Planning Binder. Selecting **Yes** notifies the Open Science Team and allows for greater tracking visibility of this manuscript within the OSP. 

:::warning
Selecting "Yes" **does not** automatically add the publication to the Planning Binder!

**It is the responsibility of the region to add the publication to the Planning Binder!**
:::


![Planning Binder](./images/planning_binder.png)

![Planning Binder Email](./images/binder_email.png)

### Confirm Review {/* #confirm-review */}

After reviewing the Manuscript Record and providing any necessary comments, you can submit your review.

To submit a Manuscript Management Review:

1. Click the **SUBMIT** button.
2. Select the decision you wish to make and click the **NEXT** button.
3. If you are forwarding the manuscript management review to another manager:
   - Click the **Next Manuscript Management Reviewer** search box and type the name of the division manager.
   - If the manager is in the database, their name will appear. Click their name to select them.
   - If their name does not appear, follow these steps to invite them to the OSP:
     1. Click the **Can't find the user you're looking for?** button.
     2. Enter the manager's **Email** and select the appropriate user from the DFO directory.
     3. Click the **INVITE** button to invite them to the OSP.
4. Click the **NEXT** button to confirm the selected manager.
5. Select **Yes** and click the **SUBMIT** button.

![Submit Decision](./images/submit_decision.png)

### Email Notifications {/* #email-notifications */}

The OSP will automatically send an email:
- When a Manuscript Management Review Action is taken.
  - Managers who have "Completed" their review will be Carbon Copied (CC'd) on all emails during the remainder of the review process.
- Once a week with a summary of all of your pending Manuscript Management Reviews.
- Two working days before the ten-working-day timeline is reached (for Third-Party Journal Publications)
